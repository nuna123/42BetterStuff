...
int	figure_fd1(int args[3], char *argv[], char **cmds[2], int pipees[2])
{
	int	res;
	...
	if (args[0] != args[1] - 2 && args[0] != args[1] - 3 && args[1] > 4)
		res = pipees[1];
	else if (!access(cmds[0][0], 0) && (!cmds[1] || !access(cmds[1][0], 0)))
	{
		if (args[2] > -1)
...
void	pipe_the_stuff(int argc, char *argv[], char *env[], int in_fd)
{
	...
	while (i < argc - 1)
	{
		...
		pipe(pipees);
		file_fds[1] = figure_fd1((int []){i, argc, in_fd}, argv, cmds, pipees);
...
int	main(int argc, char *argv[], char *env[])
{
	...
	if (ft_strncmp(argv[1], "here_doc", 8) == 0)
	{
		...
		pipe_the_stuff(argc - 2, &argv[2], env, pipees[0]);
	}
